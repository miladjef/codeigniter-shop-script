...
/*********************************************************************************
 * ****************************RC ONR.2008 Chart.JS REFERENCE *******************/
...
(this || window, function(moment) {
	...
	(function() {
		...
		helpers.niceNum = function(range, round) {
			...
			var fraction = Math.round(range / Math.pow(10, exponent));
		...
		helpers.color = function(color) {
			if (!root.Color) {
				console.log('Color.scripts not found!');
				return color;
			}
	 ...
	 * ****************************RC ONR.2008 Chart.JS REFERENCE *******************/
	/**
	 * (I guess): Working with legends and update
	 */
	(function() {
		"use strict";
	...
	(function() {
		...
		Chart.defaults.global.legend = {
			...
			labels: {
				...
				fontStyle: "normal",
				fontColor: "#477474",
				fontFamily: "BYEKAN, tahoma, arial, sans-serif",
				padding: 10,
	 ...
	 * ****************************RC ONR.2008 Chart.JS REFERENCE *******************/
	/**
	 * Gridlines , Labels and most of chart elements
	 */
	(function() {
		"use strict";
		...
		Chart.defaults.scale = {
			...
			gridLines: {
				display: true,
				color: "rgba(0, 0, 0, 0.1)",/** Color of the gridlines*/
				lineWidth: 1,
				drawOnChartArea: true,
				drawTicks: true,
				zeroLineWidth: 1,
				zeroLineColor: "rgba(0,0,255,0.3)",/** Color of the zero line*/
			...
			scaleLabel: {
				fontColor: '#008',
				fontFamily: 'BYEKAN, tahoma, arial, sans-serif',
				fontSize: 12, /** font size of X/Y labels*/
				fontStyle: 'normal',
			...
			ticks: {
				...
				fontSize: 11,
				fontStyle: "bold",
				fontColor: "#333", /** color of X/Y values labels*/
				fontFamily: "BYEKAN, tahoma, arial, sans-serif",
				maxRotation: 90,
				mirror: false,
				padding: 20, /** Padding of Y axis labels from zero gridline */
				reverse: false,
				display: true,
				autoSkip: true, /**removes not needed Y gridlines from chart automatically*/
				autoSkipPadding: 20,
	 ...
	 * ****************************RC ONR.2008 Chart.JS REFERENCE *******************/
	/**
	 * small box that shows the values when mouse hover on a point
	 */
	(function() {
		...
		Chart.defaults.global.tooltips = {
			enabled: true,
			custom: null,
			mode: 'single', /** single,label,dataset*/
			backgroundColor: "rgba(0,150,0,0.8)",
			titleFontFamily: "BYEKAN, tahoma, arial, sans-serif",
			titleFontSize: 12,
			...
			titleColor: "#FFF",
			titleAlign: "left",
			bodyFontFamily: "BYEKAN, tahoma, arial, sans-serif",
			bodyFontSize: 12,
			bodyFontStyle: "normal",
			bodySpacing: 2,
			bodyColor: "#fff",
			bodyAlign: "left",
			footerFontFamily: "BYEKAN, tahoma, arial, sans-serif",
			footerFontSize: 12,
	 ...
	 * ****************************RC ONR.2008 Chart.JS REFERENCE *******************/
	/**
	 * the settings for chart X labels(ticks)
	 */
	(function() {
		"use strict";
			...
			ticks: {
				callback: function(tickValue, index, ticks) {
					...
					if (tickValue !== 0) {
						 ...
						 *
						 * @type {number}
						 * baraye tabdile tedade ashare 20 be 1 : Math.max(Math.min(numDecimal, 20) ====tabdil shode be===>Math.max(Math.min(numDecimal, 1)
                         */
						numDecimal = Math.max(Math.min(numDecimal, 2), 0); // toFixed has a max of 20 decimal places
	...
	(function() {
		...
		var defaultConfig = {
			...
			pointLabels: {
				...
				fontFamily: "BYEKAN,tahoma, arial",
	...
	(function(moment) {
		...
		if (!moment) {
			console.warn('Chart.scripts - Moment.scripts could not be found! You must include it before Chart.scripts to use the time scale. Download at http://momentjs.com/');
			return;
		}
